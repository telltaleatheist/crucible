...
- `release` is **the release whose assets hold these bytes, which is not always the
  release whose manifest carries the row** — schema 2, added 2026-09-17.

### 2a. Why a row names a release (schema 2)

A pack is a function of its RECIPE. When a recipe and its standalone-Python pin
are unchanged, the pack the previous release built IS the pack this one would
build, and building it again spends CI minutes to arrive back where we started.

MEASURED on v0.6.6 → v0.6.7: thirteen packs, three that genuinely changed, ten
that did not. The three took 0, 0 and 1 minute. Of the ten, one `tts-higgs-v3`
rebuild ran over forty, and the release's whole wall-clock was that one pack
reproducing bytes v0.6.6 had already published. Owen, 2026-09-17: *"We don't need
to rebuild the env every time we change something."*

An unchanged pack is therefore **carried by reference — neither rebuilt nor
copied**. Copying was the obvious alternative and it is worse than it looks: a
release's assets come to 16 GB, so copying the unchanged ones forward stores the
same bytes again under every tag, forever, and moving 17 GB is not much faster
than building it. The row is carried instead and keeps naming the release that
already holds the parts. Its `parts` are still named for THAT release, which is
why the name and the row agree.

**THE INVARIANT THIS CHANGES, stated plainly.** Section 4 used to promise that a
release whose manifest exists has every pack it names *as an asset of that
release*. It now promises every pack it names **resolves**, and each row says
where. That is weaker, and it is only worth anything if somebody asks — so the
manifest job HEADs every carried part before the manifest is allowed to become
the release's own, and refuses by URL when one is missing.

**Schema 1 is still read, and that is not legacy tolerance.** Schema 1 had no
such field because it did not need one: every pack it named was an asset of its
own release, by construction. Reading those rows as the manifest's own version
therefore RECOVERS a fact that schema stated structurally — nothing is
defaulted. It has to work for one concrete reason: planning a release means
reading the PREVIOUS release's manifest, and at the moment this field arrives the
previous release is always a schema-1 one. A build that could not read it would
rebuild all thirteen packs on the very release that introduces not rebuilding
them.

A MIRROR STILL RESOLVES RELATIVE. `--manifest-url file:///srv/mirror/envpacks.json`
means "these bytes, here", so a mirror holds every part it names, exactly as
before; only a GitHub release resolves a carried row elsewhere. A mirror that
reached back to github.com for half its parts would fail on the second file for
the machine that has no route to github — which is usually why a mirror exists.
...
`envpacks.yml` runs ON THE TAG. **A `plan` job goes first** and decides which packs this
release must build at all (2a): `scripts/plan_packs.py` compares each declared pack's recipe
digest and Python pin against the newest previous release that published a manifest, and
emits the matrix rows for the ones that changed and carried rows for the ones that did not.
A previous release that cannot be read means build everything — a slow release is a fine
answer to a missing manifest, and guessing is not.

The matrix is then (pack × backend) over THAT list — `ubuntu-latest` for `cuda-linux`,
`macos-14` (arm64) for `mlx-darwin` — each job building ONE pack, smoke-testing it,
uploading its parts with `gh release upload --clobber`, and uploading its manifest fragment.
`host/llama-windows` is planned the same way but builds in a job of its own, because every
step in the matrix is written in `sh` and reclaims disk with `sudo rm -rf`, neither of which
a Windows runner does.

A final job writes the carried rows as one more fragment — they are ordinary rows, so there
is no second code path downstream — merges everything into `envpacks.json` and uploads that
LAST, so a release whose manifest exists has every pack it names. A job that fails leaves the
manifest without that pack, which `install` refuses by name (`pack_not_published`) — never a
half pack.
...
- **The manifest's shape** — schema, the fields and their types, no duplicate
  `(name, backend)`. `parse_manifest` refuses each by name and the parametrised test
  names the field. Eight fields in schema 1; nine in schema 2, which adds `release` and
  requires it there while recovering it for a schema-1 row (2a).
