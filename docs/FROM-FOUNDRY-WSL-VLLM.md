...
**Two facts the Foundry session supplied, 2026-09-15, both verified here before being written down.**
**Foundry never passed `--limit-mm-per-prompt` anywhere, ever** — grep across their `src/`, `app/electron`
and `docs/` is empty, confirmed. So the vision tower on the text lane was BookForge's flag to set and
BookForge's waste to have lost; there is no Foundry precedent on either side of it, and the fix above is
restoring BookForge to itself rather than reuniting two apps. And **`--concurrency 12` against a measured
admission of 7 is the HARMLESS direction** — vLLM admits what fits and queues the rest, so the extra
requests wait in the server rather than thrashing the card. Nobody should "fix" the 12 down to match an
admission figure; that would only idle the batch between blocks.
...
| `--mamba-cache-dtype` / `--mamba-ssm-cache-dtype` / `--kv-cache-dtype`, all `auto` | nothing | **INERT — re-verified 2026-09-15 against a specific counter-claim, and the reason is much stronger than "the default is auto".** Foundry's `docs/VLLM.md` section 7 says *"`--mamba-ssm-cache-dtype float16` is the knob that matters — it halves the state and therefore the page, and therefore roughly doubles how many sequences are admitted."* **The mechanism is right and the lever is not.** Traced in the pinned vLLM 0.29.0: `MambaSpec.page_size_bytes` <- `state_content_size_bytes` = `sum(prod(shape) * get_dtype_size(dtype))` over `dtypes`, which come from `qwen3_5.py`'s `get_mamba_state_dtype_from_config` -> `MambaStateDtypeCalculator.gated_delta_net_state_dtype` -> `_mamba_state_dtype`, where **`auto` resolves to `get_kv_cache_torch_dtype("auto", model_dtype)` = THE MODEL DTYPE**, and `mamba_ssm_cache_dtype == "auto"` copies it. Both checkpoints' configs say `dtype: bfloat16`. So the state is **already 16-bit**: `float16` is 2 bytes against `bfloat16`'s 2 bytes — no halving, no page change, no depth. `MambaDType` is `Literal["auto", "float32", "float16", "bfloat16"]`, so there is no smaller option to reach for either; 16-bit is the floor and `auto` is already on it. The "tens to hundreds of megabytes ... at fp32" figure both repos quote is the fp32 hypothetical, not what ran. (fp32 IS real for some architectures — `kda_state_dtype` pins the temporal state to `torch.float32` — but Qwen3.5/3.8 take the gated-delta-net path.) **Nothing to change and nothing to put to Owen: there is no arithmetic that yields a gain.** |
...
| `--max-model-len 16384` | `context_default = 12288` | **A NARROWING — RECOVERED AND FIXED.** Owen, 2026-09-15: *"we made the measurements previously… it was already configured to use vllm before so the configuration is in the git history."* It is. `VLLM_TEXT_MAX_MODEL_LEN:-16384` is in the launcher's FIRST commit (bookforge `cd222229`) and is unchanged through `2212708d` and `e23aabcd`; **The provenance first written here was wrong and is corrected:** 12288 is not Foundry's. Foundry's Ollama context is `DEFAULT_NUM_CTX = 8192` (`src/translate/ollama.ts`) and clean-text's ceiling is `CTX_MAX = 16384` (`src/clean/runner.ts`) — neither is 12288. It is **BookForge's own**, `electron/ai-bridge.ts`'s `numCtxMaxForModel`, which is size-tiered: 16384 for <=15B, and 12288 for **>15B or unrecognized**, "tuned for 32B Q4_K_M". So 12288 is the 32B tier and it reached a 9B — the right function's wrong tier. BookForge's own regex reads `9b` out of the id and would have returned 16384. `qwen3.5-9b`'s cuda-linux block now carries `context_default = 16384`; the model-level 12288 stays for the backends that are the Ollama/llama.cpp form. It fits by the measurement already in the manifest — the pool at util 0.84 is 27,443 tokens against a 16,384 sequence — and the tower fix widens that margin. |
