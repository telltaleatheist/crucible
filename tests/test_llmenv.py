"""The `llm` env's recipes and its status (PHASE2-LLM.md section 2).
...
from crucible import llmenv
from crucible.llmenv import (
    EnvError,
    env_status,
    llm_env_dir,
    recipe_for,
...
def test_this_build_ships_a_recipe_for_each_backend() -> None:
    assert sorted(p.stem for p in recipes_dir().glob("*.txt")) == [
        "cuda-linux",
        "mlx-darwin",
    ]
...
def test_a_backend_with_no_recipe_is_refused_by_name() -> None:
    with pytest.raises(EnvError) as caught:
        recipe_for("rocm-linux")
    assert "no llm env recipe for backend 'rocm-linux'" in str(caught.value)
    assert "['cuda-linux', 'mlx-darwin']" in str(caught.value)


def test_each_recipe_pins_its_engine_exactly() -> None:
    assert recipe_pins(recipe_for("cuda-linux"))["vllm"] == "0.29.0"
    assert recipe_pins(recipe_for("mlx-darwin"))["mlx-lm"] == "0.31.3"
...
def test_every_requirement_in_every_recipe_is_pinned() -> None:
    ...
    for backend in ("cuda-linux", "mlx-darwin"):
        pins = recipe_pins(recipe_for(backend))
...
def test_no_venv_is_not_installed(home: Path) -> None:
    status = env_status(home, "cuda-linux")
...
def test_an_env_missing_a_pinned_package_is_not_installed(
    ...
    pins = recipe_pins(recipe_for("cuda-linux"))
    short = {name: version for name, version in pins.items() if name != "torch"}
    monkeypatch.setattr(llmenv, "installed_packages", lambda _home: short)
    status = env_status(home, "cuda-linux")
    assert status.installed is False
...
def test_an_env_at_the_wrong_version_is_not_installed(
    ...
    stamp_env(home, "cuda-linux")
    pins = dict(recipe_pins(recipe_for("cuda-linux")))
    pins["torch"] = "2.5.1"
    monkeypatch.setattr(llmenv, "installed_packages", lambda _home: pins)
    status = env_status(home, "cuda-linux")
    assert status.installed is False
...
def test_a_complete_env_is_installed(
    ...
    assert require_env(home, "cuda-linux") == llm_env_dir(home) / "bin" / "python"


def test_require_env_refuses_rather_than_guessing_an_interpreter(home: Path) -> None:
    with pytest.raises(EnvError) as caught:
        require_env(home, "cuda-linux")
