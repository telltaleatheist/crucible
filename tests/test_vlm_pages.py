...
# `fake_env`, `fake_weights`, `idle_card` and `engine_factory` are in conftest.py
# and arrive on their own. `llm_client` is still declared in the phase-2 module,
# so it is imported: Ruff sees an unused import, pytest sees a fixture this module
# declares, and that is what makes it resolvable here at all. `run_job` is an
# ordinary helper and is imported for the ordinary reason.
from .test_llm_api import llm_client, run_job  # noqa: F401
