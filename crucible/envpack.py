...
#: The THIRD backend, and WINDOWS IS ONE (PHASE15 section 0's amendment, and
#: 3.5). Structurally what `mlx-darwin` is: `llama-server` children serving
#: GGUF weights, with the llm classes and `pages` answering from them. An
#: earlier draft of 4.4 called this `host-windows` and called it "a pack
#: backend, never a `backend_kind`" — that was written at a moment when
...
#:
#: **`crucible/backend.py` IS THE OWNER of this name**, beside `CUDA_LINUX`
#: and `MLX_DARWIN`, and it is spelled here only because that constant is
#: landing in the same session as this one; the two are tied by a check rather
#: than by an import for the moment. When `backend.py` exports it, this line
#: becomes an import from there and this comment goes with it.
